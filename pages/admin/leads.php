  ...
  <main class="mx-auto w-full max-w-7xl px-4 py-10">
        ...
        <table class="min-w-full text-left text-sm">
          <thead class="bg-slate-100 text-slate-600">
            <tr>
              ...
              <th class="px-4 py-3 font-semibold">Medio</th>
          ...
          <tbody class="divide-y divide-slate-100">
            ...
            <?php foreach ($rows as $row): ?>
              <tr class="hover:bg-slate-50">
                ...
                <td class="px-4 py-3 text-slate-600"><?php echo htmlspecialchars($row['medium'] ?? '', ENT_QUOTES, 'UTF-8'); ?></td>
                <td class="px-4 py-3">
                  <?php
    ...
    <section class="mt-6 flex flex-wrap items-center justify-between gap-3 text-sm text-slate-600">
      ...
      <div class="flex flex-wrap items-center gap-2">
        <?php if ($page > 1): ?>
          <a class="rounded-lg border border-slate-200 bg-white px-3 py-1.5 font-semibold text-slate-600 hover:border-slate-300" href="<?php echo $base; ?>/admin/leads?page=<?php echo $page - 1; ?>&per_page=<?php echo $perPage; ?>&from=<?php echo urlencode($dateFrom); ?>&to=<?php echo urlencode($dateTo); ?>">Anterior</a>
        <?php endif; ?>
        <?php if ($page < $totalPages): ?>
          <a class="rounded-lg border border-slate-200 bg-white px-3 py-1.5 font-semibold text-slate-600 hover:border-slate-300" href="<?php echo $base; ?>/admin/leads?page=<?php echo $page + 1; ?>&per_page=<?php echo $perPage; ?>&from=<?php echo urlencode($dateFrom); ?>&to=<?php echo urlencode($dateTo); ?>">Siguiente</a>
        <?php endif; ?>
      </div>
    </section>
  </main>
